...
export default function TakeExam() {
  ...
  // Also keep a local copy to avoid re-rendering issues
  ...
  }, []); // Empty dependency array, this will only run on mount and unmount
  
  // Fetch exam data if not already loaded
  const { data: examData, isLoading: isLoadingExam } = useQuery<any>({
    queryKey: [`/api/exams/${examId}`],
    enabled: !isNaN(examId)
  });
  
  // Update the current exam and timer only when examData changes
  useEffect(() => {
    if (examData) {
  ...
  const handleMultipleChoiceAnswer = (questionId: number, optionId: string) => {
    // Create a new Map from the current localAnswers
    const newAnswers = new Map(localAnswers);
    // Add the new answer to the Map
    newAnswers.set(questionId, optionId);
    // Update local state
    setLocalAnswers(newAnswers);
    // Also update context (outside the rendering cycle)
    setTimeout(() => {
  ...
  const goToResults = () => {
    ...
    toast({
      ...
      description: `Your score: ${submitAnswersMutation.data?.score}/${submitAnswersMutation.data?.maxScore}`,
    });
  };
  
  // Handle finish exam viewing
  const handleFinish = () => {
    // Clear context and navigate home
    clearUserAnswers();
    setCurrentExam(null);
    setCurrentAttempt(null);
    navigate("/");
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      ) : showResults && submitAnswersMutation.data ? (
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Exam Results</h1>
            <p className="text-lg text-gray-600">
              Your score: {submitAnswersMutation.data.score}/{submitAnswersMutation.data.maxScore} 
              ({Math.round((submitAnswersMutation.data.score / submitAnswersMutation.data.maxScore) * 100)}%)
            </p>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-primary">
                    {submitAnswersMutation.data.score}/{submitAnswersMutation.data.maxScore} 
                    ({Math.round((submitAnswersMutation.data.score / submitAnswersMutation.data.maxScore) * 100)}%)
                  </span>
                </div>
              </div>
                  ...
                  <button 
                    ...
                    className={`h-8 w-8 rounded flex items-center justify-center font-medium ${
                      index === currentQuestionIndex 
                        ...
                        : userAnswers.get(currentExam.questions![index].id)
