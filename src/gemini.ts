...
export async function generateExam(
  ...
  try {
    ...
    const prompt = `
      ...
      Create an exam with ${config.questionCount} ${
      config.type
...
export function generateMockExam(
  ...
  const topicLines = syllabusLines.filter(
    (line) =>
      line.includes("Topic") ||
  ...
  for (let i = 0; i < config.questionCount; i++) {
    ...
    if (config.type === "multiple-choice") {
      ...
      questions.push({
        content: `Question ${
          i + 1
        }: Briefly explain the concept of "${topic}" and its significance.`,
        questionType: "short-answer",
...
function extractCourseTitle(content: string, filename: string): string {
  ...
  const courseMatch =
    firstLines.match(/course:?\s*([a-z0-9\s]+)/i) ||
